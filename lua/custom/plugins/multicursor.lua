return {
  ...
  config = function()
    ...
    set({ 'n', 'x' }, '<C-S-c>', function()
      mc.lineAddCursor(-1)
    end, { desc = 'Add [c]ursor above main cursor' })
    set({ 'n', 'x' }, 'C', function()
      mc.lineAddCursor(1)
    end, { desc = 'Add cursor below main cursor' })
    set({ 'n', 'x' }, '<leader><up>', function()
      mc.lineSkipCursor(-1)
    end, { desc = 'Skip cursor above main cursor' })
    set({ 'n', 'x' }, '<leader><down>', function()
      mc.lineSkipCursor(1)
    end, { desc = 'Skip cursor below main cursor' })

    -- Add or skip adding a new cursor by matching word/selection
    set({ 'n', 'x' }, '<leader>n', function()
      mc.matchAddCursor(1)
    end, { desc = 'Add [n]ew cursor below by matching word/selection' })
    set({ 'n', 'x' }, '<leader>s', function()
      mc.matchSkipCursor(1)
    end, { desc = '[S]kip cursor below by matching word/selection' })
    set({ 'n', 'x' }, '<leader>N', function()
      mc.matchAddCursor(-1)
    end, { desc = 'Add [n]ew cursor above by matching word/selection' })
    set({ 'n', 'x' }, '<leader>S', function()
      mc.matchSkipCursor(-1)
    end, { desc = '[S]kip cursor above by matching word/selection' })
